...
function CreateVente({toggle,open}) {
      ...
      await sendData(
        ...
        .then(async ({ data }) => {
           console.log("response",data)
            setVenteId(data?.data?.ventes[0]?.id)
            
          //   setTimeout(()=>{document.getElementById("pdf2").click();toggle(data?.data?.ventes[0])},50
          //  )
           
           toastMsg("Vente effectuée avec succès","success")
           toggle(data?.data?.ventes[0])
  ...
  return (
    <div>
      <Modal style={{ width: '100%'}} className="my-modal" size="xl" isOpen={open} toggle={toggle}>
        ...
        <ModalHeader toggle={()=>toggle("")}>Nouvelle vente</ModalHeader>
        <ModalBody>
