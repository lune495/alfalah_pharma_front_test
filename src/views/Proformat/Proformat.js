...
export  const Proformat = () => {
    ...
    const action=(val)=>{
      ...
      return (
       <> <span onClick={()=>updateProduct(val)} style={{color:"blue",cursor:"pointer",fontSize:20,marginRight:10}} className="fa fa-pencil-square" ></span> <span onClick={()=>handleDetail(val)} style={{color:"grey",cursor:"pointer",fontSize:20,marginRight:10}} className="fa fa-info-circle" ></span>  <i onClick={()=>{setVenteId(val?.id);handleDownload(val)}} style={{color:"red",cursor:"pointer",fontSize:20}} className="fa fa-file-pdf" aria-hidden="true"></i> </>
      )
    }

    const getVentes = async (count,page,ref) => {
      const user_id = user?.id ? user?.id : 0;
      const url1 = "graphql?query={proformaspaginated(count:" + count + ",page:" + page + ",user_id:" + user_id +"){metadata{current_page per_page} data{id client{nom_complet id} numero montant_ht montant_ttc remise_total montant_avec_remise created_at proforma_produits{qte prix_vente remise montant_remise total produit{designation pv id} }  montant montant_avec_remise qte montant_ht montant_ttc remise_total user { id name} }}}"
      const url2 = "graphql?query={proformaspaginated(count:" + count + ",page:" + page + ",created_at_start:\"" + date1 + "\",created_at_end:\"" + date2 +"\"){metadata{current_page per_page} data{id client{nom_complet id} created_at numero montant_ttc remise_total montant_avec_remise proforma_produits{qte prix_vente remise montant_remise total produit{designation pv id} }  montant remise_total montant_avec_remise qte montant_ht montant_ttc user { id name} }}}"
      const url3 = "graphql?query={proformaspaginated(count:" + count + ",page:" + page + ",reference:\"" + ref +"\"){metadata{current_page per_page} data{id  client{nom_complet,id} created_at numero montant_ttc remise_total montant_avec_remise proforma_produits{qte prix_vente remise montant_remise total produit{designation pv id} }  montant montant_avec_remise remise_total qte montant_ttc montant_ht user { id name} }}}"
      setLoading(true)
      const { data } = await getData(date1 && date2? url2:ref?url3:url1);
      setNumber(
        data?.data?.proformaspaginated?.data.length!==0?data.data?.ventespaginated?.data?.length:0
        )
        if(data?.data?.proformaspaginated?.data.length){
          setIsNext(true)
        }else{
          setVente([])
          setIsNext(false)
      ...
      setVente(data?.data?.proformaspaginated.data)
      setLoading(false)
      ...
      <div className="content n-produit" style={{paddingTop:15}}>
          ...
          />
           <ModalConfirmation
                  ...
                  title="Confirmation quitter la proforma"
                  bouton="Quitter"
                  annuler="Continuer le devis"
            />
           <DetailVente
            show={open}
            detail={detailVente}
              ...
              <tbody>
                ...
                {vente.map((val,id)=>(
                  <tr style={val?.statut?{backgroundColor:"#f7d0c8"}:null}>
                    ...
                    <td>{val.client ? val.client?.nom_complet : "Passager"}</td>
